# clean_db.py
# Cleans up duplicate entries and populates 14 strictly unique, non-repeating restaurant food items.
...
def sanitize_restaurants():
    # Clear all existing restaurants to guarantee zero duplicates
    Restaurant.objects.all().delete()
    print("Database cleared. Re-seeding unique non-repeating food items...")
    ...
    for item in unique_restaurants:
        ...
        print(f"Added unique item: {item['name']}")

    print(f"Database sanitized! Total unique restaurants: {Restaurant.objects.count()}")

if __name__ == "__main__":
    sanitize_restaurants()
